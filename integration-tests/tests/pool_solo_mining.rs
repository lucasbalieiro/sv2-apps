...
#[tokio::test]
async fn pool_rejected_open_does_not_mutate_connection_payout_policy() {
    start_tracing();
    ...
    // This open must fail before it binds the connection's payout policy.
    send_to_pool
    ...
    sniffer
        ...
        .await;
    let rejected_open = next_open_channel_error(&sniffer);
    assert_ne!(
        rejected_open.error_code.as_utf8_or_hex(),
...
#[tokio::test]
async fn pool_rejects_incompatible_extended_channel_payout_policy() {
    start_tracing();
    let (tp, tp_addr) = start_template_provider(None, DifficultyLevel::Low);
    tp.fund_wallet().unwrap();
    let (pool, pool_addr, _) = start_pool(sv2_tp_config(tp_addr), vec![], vec![], false).await;
