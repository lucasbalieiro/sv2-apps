...
mod tests {
    use super::*;
    use stratum_core::bitcoin::hashes::sha256d;

    #[test]
    fn test_merkle_path_single_coinbase_tx() {
        // Should return empty path for coinbase-only block
        let coinbase_hash = sha256d::Hash::hash(&[0u8; 32]);
        let tree = MerkleTree::build(vec![coinbase_hash]);
        let path = tree.extract_coinbase_path();
        assert_eq!(path.len(), 0);
    }

    #[test]
    fn test_merkle_path_two_txs() {
        // Basic two-transaction merkle tree
        let tx1 = sha256d::Hash::hash(&[1u8; 32]);
        let tx2 = sha256d::Hash::hash(&[2u8; 32]);
        let tree = MerkleTree::build(vec![tx1, tx2]);
        let path = tree.extract_coinbase_path();
        assert_eq!(path.len(), 1);
        assert_eq!(path[0], tx2);
    }

    #[test]
    fn test_merkle_path_multiple_txs() {
        // Test with several transactions to verify path correctness
        let txs: Vec<_> = (0..8).map(|i| sha256d::Hash::hash(&[i; 32])).collect();
        let tree = MerkleTree::build(txs);
        let path = tree.extract_coinbase_path();
        assert_eq!(path.len(), 3); // log2(8) = 3
    }
}
