...
INTEGRATION_WORKSPACE="integration-tests"
...
for workspace in $ALL_WORKSPACES; do
    ...
    cargo +1.85.0 clippy --manifest-path="$workspace/Cargo.toml" -- -D warnings -A dead-code
    if [ $? -ne 0 ]; then
    ...
    cargo +1.85.0 test --manifest-path="$workspace/Cargo.toml"
    if [ $? -ne 0 ]; then
