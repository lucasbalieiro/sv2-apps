...
use super::{ALLOCATED_TOKEN_TIMEOUT_SECS, JANITOR_INTERVAL_SECS};
...
use stratum_apps::{
    ...
    stratum_core::{
        ...
        parsers_sv2::{parse_message_frame_with_tlvs, AnyMessage, JobDeclaration, Tlv},
    },
...
pub struct DownstreamIo {
    to_job_declarator_sender: Sender<(DownstreamId, JobDeclaration<'static>, Option<Vec<Tlv>>)>,
    from_job_declarator_receiver: Receiver<(JobDeclaration<'static>, Option<Vec<Tlv>>)>,
...
impl Downstream {
    ...
    pub async fn start(
        ...
        if let Err(e) = self.setup_connection_with_downstream().await {
            ...
            // todo: is it safe to ignore this? (tbh I think it's not)
            let _ = disconnect_sender.send(self.downstream_id).await;
            return;
        }
        ...
        task_manager.spawn(async move {
            ...
            let _ = disconnect_sender.send(downstream_id).await;
        });
    }
