...
impl TokenManager {
    ...
    pub fn is_active(&self, token: JdToken) -> bool {
        self.active_tokens.contains_key(&token)
    }

    /// Checks if an allocated token has been activated (has a corresponding active token).
    pub fn has_active_for_allocated(&self, allocated_token: JdToken) -> bool {
        self.active_tokens
            .iter()
            .any(|entry| entry.value().0 == allocated_token)
    }
